...
Connect to github.com (127.0.0.1) on port 80, using a SOCKS proxy.
...
const options = {
  ...
  destination: {
    host: '127.0.0.1', // github.com (hostname lookups are supported with SOCKS v4a and 5)
    port: 80
  }
};
...
try {
  ...
  info.socket.on('data', (data) => {
    console.log(data.toString()); // ip-api.com sees that the last proxy in the chain (127.0.0.1) is connected to it.
    /*
      HTTP/1.1 200 OK
...
const options = {
  ...
  // When using BIND, the destination should be the remote client that is expected to connect to the SOCKS proxy. Using 127.0.0.1 makes the Proxy accept any incoming connection on that port.
  destination: {
    host: '127.0.0.1',
    port: 0
  }
...
const options = {
  ...
  // When using associate, the destination should be the remote client that is expected to send UDP packets to the proxy server to be forwarded. This should be your local ip, or optionally the wildcard address (127.0.0.1)  UDP Client <-> Proxy <-> UDP Client
  destination: {
    host: '127.0.0.1',
    port: 0
  }
...
udpSocket.on('message', (message, rinfo) => {
  ...
  /*
  { frameNumber: 0,
    remoteHost: { host: '127.0.0.1', port: 4444 }, // The remote host that replied with a UDP packet
...
```typescript
{
  ...
  destination: {
    host: '127.0.0.1', // ipv4, ipv6, hostname. Hostnames work with v4a and v5.
    port: 80
  },
...
const options = {
  ...
  destination: {
    host: '127.0.0.1', // ipv4, ipv6, hostname
    port: 80
  }
}
```
...
```typescript
{
  ...
  remoteHost: {
    host: '127.0.0.1', // The remote host that is listening (usually the proxy itself)
