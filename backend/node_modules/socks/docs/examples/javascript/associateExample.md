...
udpSocket.on('message', (message, rinfo) => {
  ...
  /*
  { frameNumber: 0,
    remoteHost: { host: '127.0.0.1', port: 53 }, // The remote host that replied with a UDP packet
...
const options = {
  ...
  // Most SOCKS servers accept 127.0.0.1 as a wildcard address to accept UDP frames from any source.
  destination: {
    host: '127.0.0.1',
    port: 0
  },
...
client.on('established', info => {
  ...
  */
  ...
  // Send a udp frame to 127.0.0.1 on port 53 through the proxy.
  const packet = SocksClient.createUDPFrame({
    remoteHost: { host: '127.0.0.1', port: 53 },
